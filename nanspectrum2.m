...
D1 = DD(1); D2 = DD(2);
SEG1 = SEGMENTS(1); SEG2 = SEGMENTS(2);
...
[F1,F2] = meshgrid(F1(1:[end-1]),F2(1:[end-1]));
...
if PLOT_BOOLEAN
    % figure
    % imagesc(unique(F1),unique(F2),log10(SPEC))
    % title('imagesc')
    % clim([prctile(log10(SPEC(:)),1) max(log10(SPEC(:)))])

    figure
    pcolor_centered(F2,F1,log10(SPEC)); shading flat
    title(['Power spectrum units: [(time\_series\_units)^2 (cycles per ' UNITS{1} ')^{-1} (cycles per ' UNITS{2} ')^{-1}]'])
    clim([prctile(log10(SPEC(:)),1) max(log10(SPEC(:)))])
    ...
    % ax = gca; ax.YDir = 'normal'; % for imagesc
